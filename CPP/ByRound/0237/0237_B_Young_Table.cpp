/*
 * Problem URL : https://codeforces.com/problemset/problem/237/B
...
int main() {
  ...
  for (int row = 1; row <= n; row++) {
    for (int col = 1; col <= c[row]; col++) {
      ...
      swaps.push_back(
          std::pair<std::pair<int, int>, std::pair<int, int>>(source, target));
  ...
  printf("%lu\n", swaps.size());
  for (int p = 0; p < swaps.size(); p++) {
    printf("%d %d %d %d\n", swaps[p].first.first, swaps[p].first.second,
           swaps[p].second.first, swaps[p].second.second);
  }
