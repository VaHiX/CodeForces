 ...
 * 
 * This program calculates the sum of digits of numbers from 1 to n, where n can be very large.
 * It uses mathematical patterns to efficiently compute the sum without iterating through each number.
 * 
 * Approach:
 ...
 */
 ...
 /**
  * Calculates the sum of digits for numbers from 1 to the number containing the nth digit
  * @param n The position of the digit to find
  */
 void calculateDigitSum(ll n) {
	 // Initialize variables to track the current range
	 ll numbersInRange = 9;    // Number of numbers in current range (1-digit: 9, 2-digit: 90, etc.)
	 ...
	 for (int i = 0; i < numberStr.size(); i++) {
		 ...
		 digitSum += currentDigit * (2 * prefixSum + currentDigit - 1) / 2 * powerOfTen;
