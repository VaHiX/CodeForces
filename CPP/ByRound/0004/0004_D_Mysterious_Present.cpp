...
int main() {
  ...
  for (long p = 0; p < totalEnv; p++) {
    for (long q = p + 1; q < totalEnv; q++) {
      if ((envVec[p].width < envVec[q].width) &&
          (envVec[p].height < envVec[q].height) &&
          (endingLength[q] < endingLength[p] + 1)) {
        previous[q] = p;
        endingLength[q] = endingLength[p] + 1;
        if (endingLength[q] > maxLength) {
          maxLength = endingLength[q];
          maxFinal = q;
        }
      }
    }
  ...
  long current = maxFinal;
  for (long k = 0; k < maxLength; k++) {
    usedEnv[maxLength - k - 1] = envVec[current].order;
