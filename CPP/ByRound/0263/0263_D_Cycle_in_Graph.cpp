/*
 * Problem URL : https://codeforces.com/contest/263/problem/D
...
void dfs(long node, const std::vector<std::vector<long>> &g,
         std::vector<long> &path, std::vector<bool> &been) {
...
int main() {
  ...
  long last = path.back();
  std::set<long> ns;
  for (long p = 0; p < g[last].size(); p++) {
    ns.insert(g[last][p]);
  }

  long start(0);
  while (!ns.count(path[start])) {
    ++start;
  }
  printf("%ld\n", path.size() - start);
  for (long p = start; p < path.size(); p++) {
    printf("%ld ", path[p]);
  }
  puts("");
