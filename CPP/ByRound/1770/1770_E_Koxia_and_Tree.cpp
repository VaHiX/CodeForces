...
/*
 * Code Purpose: Computes the expected distance between two randomly chosen butterflies
 *               on a tree after a random orientation process of edges and butterfly movements.
 *
 * Algorithms/Techniques:
 * - Tree DFS traversal to compute subtree sizes.
 * - Dynamic Programming on Tree to track probabilities of butterfly presence at nodes.
 * - Linearity of expectation used to compute the final expected value.
 *
 * Time Complexity: O(n)
...
int main() {
  ...
  for (int i = 1; i < n; i++) {
    ...
    ans += ((ll)A * B + (ll)C * D + (ll)A * D % M * I + (ll)B * C % M * I) % M *
           X % M;
