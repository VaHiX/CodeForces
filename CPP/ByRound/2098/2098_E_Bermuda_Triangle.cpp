/*
 * Problem URL : https://codeforces.com/problemset/problem/2098/E
...
void solve() {
  ...
  // cout<<z<<" "<<euclidZ.first.second<<"\n";
  z *= (euclidY.first.first - euclidX.first.first) / gc;
  ll time = euclidX.first.first + (z * (n / an));
  time %= n / an / gc * (n / bn);
  // cout<<euclidZ.first.first<<" "<<(euclidY.first.first-euclidX.first.first) /
  // gc<<" "<<z<<" "<<(z * n / an)<<"\n";
  if (time < 0) {
    time += n / an / gc * (n / bn);
  }
  ...
  // cout << time << " " << one << " " << two << "\n";
  // cout<<(time * vx + n - x)%n<<" "<<(time * vy + n - y) %n<<"\n";
