...
template <typename A> using vc = vector<A>;
inline int read() {
  int s = 0, w = 1;
  char ch;
...
inline void run() {
  ...
  for (int i = 1; i <= n; i++)
    if (!vis[i]) {
      int p = id[i], ans = 0;
      for (int j = 65536; j; j >>= 1)
        if (ans + j <= len && y[p] > dp[ans + j])
          ans += j;
      if (ans == len)
        len++;
      dp[ans + 1] = y[p], nod[ans + 1].push_back(i);
      if (ans)
        lst[i] = nod[ans][nod[ans].size() - 1];
      else
        lst[i] = 0;
    }
  cnt++, l[cnt] = ans.size();
  ...
  if (ma < len) {
    ...
    for (int i : nod[w])
      vis[i] = 1, ans.push_back(id[i]);
  }
  r[cnt] = ans.size() - 1;
}
inline bool check(int a, int b, int c) {
  return (ll)(x[a] - x[b]) * (x[c] - x[b]) + (ll)(y[a] - y[b]) * (y[c] - y[b]) >
         0;
}
inline int get() {
  int val = 0;
  for (unsigned i = 1; i + 1 < ans.size(); i++)
    val += check(ans[i - 1], ans[i], ans[i + 1]);
  return val;
}
int p[80001];
inline void init() {
...
inline void HASH(int l, int r) {
  ...
  for (int i = 1; i <= n; i++)
    pp[i] = pp[i - 1] * b % mod;
  for (int i = 0; i < n; i++)
    h[0] = (h[0] + pp[i] * ans[i]) % mod;
  ll vd =
      (ans[1] * pp[2] + ans[2] * pp[1] - ans[1] * pp[1] - ans[2] * pp[2]) % mod;
  vd = (vd % mod + mod) % mod;
    ...
    if (r[i] - l[i] >= 4) {
      ...
      if (i != 1) {
        ...
        for (int j = l[i] + 2; j <= r[i]; j++)
          if (check(ans[r[i - 1]], ans[l[i]], ans[j]) != val)
            lf[i] = l[i] + 1;
      }
      if (i != cnt) {
        int val = check(ans[r[i] - 1], ans[r[i]], ans[l[i + 1]]);
        for (int j = l[i]; j <= r[i] - 2; j++)
          if (check(ans[j], ans[r[i]], ans[l[i + 1]]) != val)
            rf[i] = r[i] - 1;
      }
      if ((rf[i] - lf[i]) % 2 == 0)
        lf[i]++;
