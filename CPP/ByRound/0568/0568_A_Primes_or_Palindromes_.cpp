/*
 * Problem URL : https://codeforces.com/contest/568/problem/A
...
int main() {
  ...
  for (long p = 1; p <= N; p++) {
    primeCount[p] = primeCount[p - 1] + isPrime[p];
  }

  std::vector<bool> isPal(N + 1, 0);
  for (long p = 1; p <= N; p++) {
    isPal[p] = isPalindrome(p);
  }

  std::vector<long> palCount(N + 1, 0);
  for (long p = 1; p <= N; p++) {
    palCount[p] = palCount[p - 1] + isPal[p];
  }

  long p, q;
  scanf("%ld %ld\n", &p, &q);
  for (long k = N; k >= 1; k--) {
    if (p * palCount[k] >= q * primeCount[k]) {
      printf("%ld\n", k);
      return 0;
    }
  }
