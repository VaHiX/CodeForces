// Problem: CF 1656 I - Neighbour Ordering
// https://codeforces.com/contest/1656/problem/I

/*
 * Problem: Neighbour Ordering
 * Algorithm: This code determines if a graph has a "good neighbor ordering",
 * which is an ordering of neighbors for each vertex such that for every simple cycle,
 * the ordering is either clockwise or counter-clockwise consistent.
 * 
 * Techniques:
 * 1. Use of Union-Find with path compression for finding bridge edges in graph
 * 2. DFS for finding biconnected components and bridge detection
 * 3. Sorting edges using quicksort with randomized pivot
 * 4. Graph traversal technique to build valid neighbor lists
 * 
 * Time Complexity: O((V + E) * log E) where V is vertices and E is edges
 * Space Complexity: O(V + E) for storing graph and auxiliary data structures
 */
...
int ii[M], jj[M], ds[M];
char tree[M];
int *eh[N], eo[N], pp[N], ta[N], tb[N], prev[N], next[N], xx[N], _;
char in[N];
...
int compare_lr(int h1, int h2) {
  return xx[ii[h1]] != xx[ii[h2]] ? xx[ii[h1]] - xx[ii[h2]]
                                  : xx[jj[h2]] - xx[jj[h1]];
}
int (*compare)(int, int);
...
int main() {
  ...
  while (t--) {
    ...
    memset(in, 0, n * sizeof *in), memset(prev, -1, n * sizeof *prev),
        memset(next, -1, n * sizeof *next);
    yes = 1;
    for (h = 0; h < m; h = h_) {
      h1 = find(hh[h]), h_ = h + 1;
