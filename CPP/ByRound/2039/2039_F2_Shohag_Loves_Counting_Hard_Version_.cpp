/*
 * Problem URL : https://codeforces.com/contest/2039/problem/F2
...
int main(){
    ...
    for(int i=1;i<=N;i++)gr[i]+=gr[i-1];
    pr[0]=gr[N];
    for(int i=1;i<=N;i++)pr[i]+=pr[i-1];
    for(int i=1;i<=N;i++)gl[i]=gr[i-1],pl[i]=pr[i-1];
    ...
    for(int i=1;i<=N;i++){
        ans[i]=ans[i-1];
        for(int t=gr[i-1]+1;t<=gr[i];t++)as[id[t]]=tp[id[t]];
        ...
        for(int t=gr[i-1]+1;t<=gr[i];t++){
            int j=id[t];
            ad(as[j],mod-f[j]);
            ad(as[j],as[j]);ad(as[j],1);
            ad(f[j],as[j]);
            if(j==i)ad(ans[i],as[i]);
        }
        for(int t=pr[i-1]+1;t<=pr[i];t++){
            int z=id[t],o=i/z;
