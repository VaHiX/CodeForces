/*
 * Problem URL : https://codeforces.com/contest/575/problem/A
...
void build(int l, int r, int t) {
  if (l == r)
    return (void)(f[t] = matrix(s[l + 1], s[l]));
  int mid = (l + r) >> 1, k = t << 1;
...
int main() {
  ...
  for (int i = 0; i < m; ++i) {
    if (now < (x = a[i].first))
      ans *= calc(now - 1, x - 2), now = x;
    if (now == x)
      ans *= matrix(a[i].second, s[(x - 1) % n]), ++now;
    ans *= matrix((a[i + 1].first == x + 1 && i < m - 1 ? a[i + 1].second
                                                        : s[(x + 1) % n]),
                  a[i].second);
    ++now;
  }
