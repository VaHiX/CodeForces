/*
 * Problem URL : https://codeforces.com/contest/370/problem/B
...
bool checkSubset(std::vector<int> a, std::vector<int> b) {
  ...
  cutEnd = std::set_intersection(a.begin(), a.end(), b.begin(), b.end(),
                                 cut.begin());
...
int main() {
  ...
  for (int first = 0; first < n; first++) {
    ...
    for (int second = 0; second < n; second++) {
      ...
      if (checkSubset(numbers[first], numbers[second])) {
        subsetFlag = 1;
        break;
      }
    }
