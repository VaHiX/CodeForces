/*
 * Problem URL : https://codeforces.com/contest/380/problem/C
...
struct segtree {
  ...
  data merge(const data &a, const data &b) {
    ...
    ll diffA = a.open - a.paired / 2;
    diffA = (diffA > 0LL) ? diffA : 0LL;
    ll diffB = b.close - b.paired / 2;
    diffB = (diffB > 0LL) ? diffB : 0LL;
  ...
  void set(ll idx, data val, ll node, ll lv, ll rv) {
    ...
    ll mid = (lv + rv) / 2;
    if (idx < mid) {
      set(idx, val, 2 * node + 1, lv, mid);
    } else {
      set(idx, val, 2 * node + 2, mid, rv);
    }
