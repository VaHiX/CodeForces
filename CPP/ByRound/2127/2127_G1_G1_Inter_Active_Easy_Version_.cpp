...
int ask() {
    cout << "? ";
    for (int i = 1; i <= n; i++) cout << qarr[i] << ' ';
    cout << endl; // endl flushes (required)
    int r; 
    if (!(cin >> r)) exit(0);       // safety
    if (r == -1) exit(0);           // per statement
    return r;
}
...
void build_query(int fix, vector<int> good, vector<int> bad) {
    ...
    for (int i = 1; i < k && !good.empty(); i++) {
        qarr[i] = good.back(); used[qarr[i]] = true; good.pop_back();
    }
    ...
    for (int i = k + 1; !bad.empty(); i++) {
        qarr[i] = bad.back(); used[qarr[i]] = true; bad.pop_back();
    }
    // fill any leftover spots with remaining numbers
    vector<int> rest;
    for (int v = 1; v <= n; v++) if (!used[v]) rest.push_back(v);
    for (int i = 1; i <= n; i++) if (qarr[i] == 0) {
        qarr[i] = rest.back(); rest.pop_back();
    }
}
...
void rev_except_k() {
    vector<int> v;
    v.reserve(n-1);
    for (int i = 1; i <= n; i++) if (i != k) v.push_back(qarr[i]);
    for (int i = 1, j = (int)v.size()-1; i <= n; i++) if (i != k) {
        qarr[i] = v[j--];
    }
}
...
void solve_one() {
    ...
    for (int target = 1; target <= n; target++) {
        ...
        vector<int> cand;
        cand.reserve(n-1);
        for (int v = 1; v <= n; v++) if (v != target) cand.push_back(v);
