// Problem: CF 1737 G - Ela Takes Dancing Class
// https://codeforces.com/contest/1737/problem/G

// # Dancing Class Problem Solution
// ## Algorithm: Persistent Segment Tree with Lazy Propagation
// ## Time Complexity: O(n log n + q log^2 MAX_COORD)
...
struct node {
  ...
  node(ll x) : rd(rnd()), x(x) { ch[0] = ch[1] = 0; }
  void pushtag(ll k) {
    x += k;
    add += k;
  }
...
ll query(int x) {
  if (upper_bound(a2 + 1, a2 + n2 + 1, a[x]) - a2 - 1 + rt->s < x)
    return a[x];
  ll l = 1, r = 3e18;
  while (l < r) {
    ll mid = l + (r - l) / 2;
...
int main() {
  ...
  sort(ord + 1, ord + m + 1, [](int a, int b) { return x[a] < x[b]; });
  for (int i = 1; i <= n; i++)
    if (s[i] == '0')
      a2[++n2] = a[i];
  int c = 1;
  ...
  for (int i = 1; i <= n; i++)
    if (s[i] == '1') {
      ...
      // Process all queries that happen before this movable point moves past
      // current position
      while (c <= m && x[ord[c]] - t <= calc(u))
        deal(ord[c++]);
      move(calc(u));
      rt = merge(rt, new node(u));
    }
  // Process remaining queries after all movable points are handled
  while (c <= m)
    deal(ord[c++]);
  for (int i = 1; i <= m; i++)
    printf("%lld\n", ans[i]);
