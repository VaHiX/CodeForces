/*
 * Problem URL : https://codeforces.com/contest/2013/problem/F1
	...
	while(T--){
		...
		while(now){
			...
			for(int i=head[now];i;i=pre(i)){
				int l=to(i);
				if(l==pre||l==fa[now])continue;
				ans[now]=max(ans[now],mdep[l]-dep[now]);
			}
			pre=now;
			now=fa[now];
		}
		...
		for(int i=N;i<ct;++i){
			int l=i,r=2*N-i;
			if(ct%2==1)r--;
			B[l]=max(dep[u]-dep[q[l]]+ans[q[l]],dep[u]-dep[q[r]]+ans[q[r]]);
			if(l>N)B[l]=max(B[l],B[l-1]);
		}
		for(int i=N;i>1;--i){
			int l=i,r=2*N-i;
			if(ct%2==0)r++;
			A[l]=max(dep[q[l]]-1+ans[q[l]],dep[q[r]]-1+ans[q[r]]);
			if(l<N)A[l]=max(A[l],A[l+1]);
		}
		int L=ct,R=1;
		while(1){
			if(dep[q[L]]-1+ans[q[L]]>B[L-1]){
				flag=1;
				break;
			}
			else if(L-1<=R){
		...
		for(int i=1;i<=n;++i)ans[i]=head[i]=0;
		cnt=0;
	}
}
