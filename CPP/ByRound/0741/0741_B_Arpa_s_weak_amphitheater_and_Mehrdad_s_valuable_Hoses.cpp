/*
 * Problem URL : https://codeforces.com/problemset/problem/741/B
...
int main() {
  ...
  for (long p = 0; p < n; p++) {
    ...
    for (long ww = W; ww >= 0; ww--) {
      if (sw[p] <= ww) {
        f[ww] = std::max(f[ww], f[ww - sw[p]] + sb[p]);
      }
      for (long t = 0; t < sv[p].size(); t++) {
        long u = sv[p][t];
        if (w[u] <= ww) {
          f[ww] = std::max(f[ww], f[ww - w[u]] + b[u]);
        }
      }
    }
  }
