...
void printVec(std::vector<long> vv) {
...
bool checkSubset(std::vector<long> d, std::vector<long> u, long minSum,
                 long maxSum, long rel) {
  ...
  return ((subset.back() - subset[0] >= rel) && (getSum(subset) >= minSum) &&
          (getSum(subset) <= maxSum));
}
