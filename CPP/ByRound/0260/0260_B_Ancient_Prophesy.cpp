/*
 * Problem URL : https://codeforces.com/contest/260/problem/B
...
int main() {
  ...
  for (size_t k = 0; k <= input.size() - L; k++) {
    ...
    if (current[1] < '0' || current[0] > '9') {
      continue;
    }
    if (current[3] < '0' || current[3] > '1') {
      continue;
    }
    if (current[4] < '0' || current[4] > '9') {
      continue;
    }
    if (current[6] != '2' || current[7] != '0' || current[8] != '1') {
      continue;
    }
    if (current[9] < '3' || current[9] > '5') {
      continue;
    }
    ...
    if (std::lexicographical_compare(currentMonth, currentMonth + 2, minMonth,
                                     minMonth + 2) ||
        std::lexicographical_compare(maxMonth, maxMonth + 2, currentMonth,
                                     currentMonth + 2)) {
      continue;
    }
    ...
    if (std::lexicographical_compare(currentDays, currentDays + 2, minDays,
                                     minDays + 2) ||
        std::lexicographical_compare(maxDays, maxDays + 2, currentDays,
                                     currentDays + 2)) {
      continue;
    }

    if (dates.find(current) != dates.end()) {
      ++dates[current];
    } else {
      dates.insert(std::pair<std::string, int>(current, 1));
    }
  }
