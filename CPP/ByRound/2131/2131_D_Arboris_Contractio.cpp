 ...
 *
 * This program solves a tree problem where we need to find the minimum number
 * of edges to remove so that all remaining connected components are stars
 * (trees with diameter at most 2).
 *
 * Approach:
 * 1. The problem reduces to finding the maximum number of star centers in the
 * tree.
 * 2. A star center is a node connected to multiple leaves.
 * 3. The answer is (number of leaves - maximum number of leaves connected to
 * any single node).
 *
