/*
 * Problem URL : https://codeforces.com/contest/364/problem/A
...
int main() {
  ...
  long long total(0);
  for (long long p = 0; p < input.size(); p++) {
    total += (input[p] - '0');
    sum[p + 1] = total;
  }
  ...
  if (a > 0) {
    for (long p = 1; p * p <= a; p++) {
      if (p >= counts.size() || (a / p) >= counts.size()) {
        continue;
      }
      if (a % p == 0) {
        output += ((p * p == a) ? 1 : 2) * counts[p] * counts[a / p];
      }
    }
  } else {
    output = counts[0] * (input.size() * (input.size() + 1) - counts[0]);
  }
