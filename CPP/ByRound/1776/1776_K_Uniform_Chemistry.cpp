/*
 * Problem URL : https://codeforces.com/contest/1776/problem/K
...
int n;
Vec F[N], S[N];
// double Powsum(int t,int w) { double res=0; For(k,1,t) res+=pow(k,-w);; return
// res; }
const int M = 5e5;
Vec Pre[A];
void InitPowSum() {
...
signed main() {
  ...
  For(i, 1, n) {
    ...
    F[i] = Exp(F[i]);
    S[i].resize(A);
    partial_sum(F[i].begin(), F[i].end(), S[i].begin());
  }
  For(i, 1, n) {
    double ans = F[i][0];
    For(j, 1, A - 1) {
      double tt = F[i][j];
      For(k, 1, n) if (k != i) tt *= (1 - S[k][j - 1]);
      ;
      ans += tt;
    }
