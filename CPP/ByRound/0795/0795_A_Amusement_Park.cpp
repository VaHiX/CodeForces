/*
 * Problem URL : https://codeforces.com/contest/795/problem/A
...
int32_t main() {
  ...
  while (t--) {
    ...
    string s;
    cin >> s;
    auto solve = [&](int g) -> int {
      int each = (n / g);
      int rem = n % g;
      int y = DO(each) * (g - rem) + DO(each + 1) * rem;
      // cout<<y<<endl;
      y -= 2 * n;
      y += g;
      int ans = g * c1;
    ...
    int cnt = 0;
    fo(i, 0, n - 1) { cnt += (s[i] == '1'); }
    int ans = INF;
    fo(i, 1, cnt) { ans = min(ans, solve(i)); }
    cout << ans << endl;
  }
  return 0;
}