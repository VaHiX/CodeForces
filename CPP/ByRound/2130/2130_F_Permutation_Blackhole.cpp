 ...
 * 
 * Problem: Count the number of valid permutations where each element's value equals
 * the length of the longest increasing or decreasing subsequence ending at that position.
 * 
 * Approach: Dynamic Programming with combinatorial counting
 * Time Complexity: O(n^4 * k^3) where n is the maximum array size (100) and k is the maximum value (11)
 * Space Complexity: O(n^2 * k^2) for the DP tables
 */
...
ll n;
ll dp[MAX_N][MAX_N][MAX_K][MAX_K];  // dp[l][r][x][y]: count for interval [l,r] with x left and y right constraints
...
void solve() {
    ...
    for (int len = 1; len <= n + 1; ++len) {
        for (int left = 0; left + len <= n + 1; ++left) {
            ...
            for (int left_constraint = 0; left_constraint < MAX_K; ++left_constraint) {
                for (int right_constraint = 0; right_constraint < MAX_K; ++right_constraint) {
                    ...
                    if (left + 1 == right) {
                        ...
                        sum_left[left][right][left_constraint] = 
                            (sum_left[left][right][left_constraint] + dp[left][right][left_constraint][right_constraint]) % MOD;
                        sum_right[left][right][right_constraint] = 
                            (sum_right[left][right][right_constraint] + dp[left][right][left_constraint][right_constraint]) % MOD;
                        continue;
                    }
                    ...
                    for (int split = left + 1; split <= right - 1; ++split) {
                        if (sequence[split] != -1) {
                            ...
                            for (int left_part = 0; left_part <= sequence[split]; ++left_part) {
                                int right_part = sequence[split] - left_part;
                                if ((split - left <= right - split && left != 0) || right == n + 1) {
                                    // Case 1: Left side is smaller or equal to right side
                                    if (left_constraint > 0) {
                                        count = (count + 
                                            comb[right - left - 2][split - left - 1] * 
                                            dp[left][split][left_constraint - 1][left_part] % MOD * 
                                            dp[split][right][right_part][right_constraint]) % MOD;
                                    }
                                } else {
                                    ...
                                    if (right_constraint > 0) {
                                        count = (count + 
                                            ...
                                            dp[split][right][right_part][right_constraint - 1]) % MOD;
                                    }
                                }
                            }
                        } else {
                            ...
                            if ((split - left <= right - split && left != 0) || right == n + 1) {
                                ...
                                if (left_constraint > 0) {
                                    count = (count + 
                                        comb[right - left - 2][split - left - 1] * 
                                        sum_left[left][split][left_constraint - 1] % MOD * 
                                        sum_right[split][right][right_constraint]) % MOD;
                                }
                            } else {
                                // Case 2: Right side is smaller
                                if (right_constraint > 0) {
                                    count = (count + 
                                        ...
                                        sum_right[split][right][right_constraint - 1]) % MOD;
                                }
                            }
                        }
                    }
