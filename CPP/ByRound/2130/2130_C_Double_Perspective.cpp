 ...
 *
 * Problem: Given a set of intervals, where each interval has a start and end
 * point, we need to find for each unique start point the interval with the
 * maximum end point, and output their original indices.
 *
 * Approach:
 * 1. For each test case:
 *    a. Read the number of intervals
 *    b. For each interval, store the maximum end point for each start point
 *    c. Keep track of the original index of the interval with the maximum end
 * point d. Output the count of unique start points and their corresponding
 * interval indices
 *
...
void solveTestCase() {
  ...
  for (int originalIndex = 1; originalIndex <= numIntervals; ++originalIndex) {
    ...
    // Update if this is the first occurrence of 'start' or if it has a larger
    // end point
