...
inline int addmod(int a, int b) { a += b; return a >= MOD ? a - MOD : a; }
inline int submod(int a, int b) { a -= b; return a < 0 ? a + MOD : a; }
...
void preprocess(int n) {
    ...
    for (int i = n; i >= 1; i--) invfact[i - 1] = (LL)invfact[i] * i % MOD;
}
...
int main() {
    ...
    while (T--) {
        ...
        res = addmod(res, res);
        if (n & 1) inc(res, modexp(2, n));
        else dec(res, modexp(2, n));
        cout << res << "\n";
    }
    return 0;
}
