 ...
 * @brief Solution for Codeforces problem 2130E2 - Interactive RBS (Medium Version)
 * 
 * Problem: Given an interactive problem where we need to determine a valid Regular Bracket Sequence (RBS)
 * of length n, where each character is either '(' or ')' and the sequence is properly nested.
 * 
 * Approach: 
 * 1. Find a pivot point where the sequence first becomes unbalanced using binary search
 * 2. Process the sequence in chunks of 7 characters to minimize the number of queries
...
void solveTestCase() {
    ...
    int pivotPos = (firstUnbalancedPos == sequenceLength + 1) ? 1 : firstUnbalancedPos;
    ...
    for (int chunkStart = 1; chunkStart <= sequenceLength; chunkStart += CHUNK_SIZE) {
        ...
        for (int posInChunk = 0; posInChunk < chunkLength; ++posInChunk) {
            // The least significant bit tells us if this is an opening or closing bracket
            if (queryResult & 1) {
                bracketSequence += "(";
            } else {
                bracketSequence += ")";
