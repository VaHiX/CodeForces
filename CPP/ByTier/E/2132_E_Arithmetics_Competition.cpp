 ...
 *
 * This program solves a problem where we need to find the maximum sum of
 * elements by selecting a total of 'k' elements from two arrays 'a' and 'b'
 * with constraints on the maximum number of elements that can be taken from
 * each array.
 *
 * Approach:
 * 1. Sort both arrays in descending order
 * 2. Compute prefix sums for both arrays
 * 3. For each query, use ternary search to find the optimal split between the
 * two arrays
 *
