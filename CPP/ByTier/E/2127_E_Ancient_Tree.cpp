...
void dfst(int u,int tp){
	dfn[u]=++num,top[u]=tp;
	if(son[u]) dfst(son[u],tp);
	for(int v:e[u]) if(v!=fa[u]&&v!=son[u]) dfst(v,v);
}
int lca(int u,int v){
	while(top[u]!=top[v])
...
void dfsc(int u){
	if(!c[u]) c[u]=c[fa[u]];
	for(int v:e[u]) if(v!=fa[u]) dfsc(v);
}
int main(){
	...
	for(cin>>T;T--;){
		...
		dfsc(1);
		long long ans=0;
		for(int i=1;i<=n;++i) if(tag[i]) ans+=w[i];
		cout<<ans<<'\n';
