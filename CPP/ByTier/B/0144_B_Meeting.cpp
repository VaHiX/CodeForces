...
int main() {
  ...
  for (long k = ymin; k <= ymax; k++) {
    coldGenerals.insert(std::pair<long, long>(xmin, k));
  }
  for (long k = ymin; k <= ymax; k++) {
    coldGenerals.insert(std::pair<long, long>(xmax, k));
  }
  for (long k = xmin; k <= xmax; k++) {
    coldGenerals.insert(std::pair<long, long>(k, ymin));
  }
  for (long k = xmin; k <= xmax; k++) {
    coldGenerals.insert(std::pair<long, long>(k, ymax));
  }
  ...
  long numRadiators(0);
  scanf("%ld", &numRadiators);

  for (long k = 0; k < numRadiators; k++) {
    long xtemp(0), ytemp(0), tempRadius(0);
    scanf("%ld %ld %ld", &xtemp, &ytemp, &tempRadius);
  ...
  for (long k = 0; k < numRadiators; k++) {
    ...
    while (myIter != coldGenerals.end()) {
      if (isClose(myIter->first, myIter->second, radiators[k].first,
                  radiators[k].second, radii[k])) {
        coldGenerals.erase(myIter);
        myIter = coldGenerals.begin();
      } else {
        myIter++;
      }
