/*
 * Problem URL : https://codeforces.com/problemset/problem/304/B
...
int main() {
  ...
  double order =
      10000.0 * (yearB - yearA) + 100.0 * (monthB - monthA) + (dayB - dayA);
  ...
  for (int currentYear = yearA; currentYear <= yearB; currentYear++) {
    ...
    if (currentYear == yearA) {
      for (int currentMonth = 1; currentMonth < monthA; currentMonth++) {
        if (currentMonth == 2 && currentLength == 366) {
          total -= 29;
        } else {
          total -= monthDays[currentMonth];
        }
      }
      total -= dayA;
    }
    ...
    if (currentYear == yearB) {
      for (int currentMonth = monthB; currentMonth <= 12; currentMonth++) {
        total -= monthDays[currentMonth];
      }
      total += dayB;
      if (currentLength == 366 && monthB <= 2) {
        --total;
      }
    }
  }
