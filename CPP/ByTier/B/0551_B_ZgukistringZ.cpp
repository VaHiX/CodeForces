/*
 * Problem URL : https://codeforces.com/contest/551/problem/B
...
int main() {
  ...
  for (size_t p = 0; p < N; p++) {
    if (bcount[p] > 0 && (acount[p] / bcount[p]) < maxB) {
      maxB = (acount[p] / bcount[p]);
    }
  }
  ...
  for (size_t b = 0; b <= maxB; b++) {
    size_t candC(a.size());
    for (size_t p = 0; p < N; p++) {
      if (ccount[p] > 0 && ((acount[p] - b * bcount[p]) / ccount[p]) < candC) {
        candC = (acount[p] - b * bcount[p]) / ccount[p];
      }
    }
    if (b + candC > maxSum) {
      maxSum = b + candC;
  ...
  for (int p = 0; p < optB; p++) {
    std::cout << b;
  }
  for (int p = 0; p < optC; p++) {
    std::cout << c;
  }
  for (int p = 0; p < N; p++) {
