/*
 * Problem URL : https://codeforces.com/contest/254/problem/B
...
int main() {
  ...
  while (n--) {
    ...
    for (int lag = 1; lag <= t; lag++) {
      ...
      if (countMap.find(workDay) == countMap.end()) {
        countMap.insert(std::pair<int, long>(workDay, p));
      } else {
        countMap[workDay] += p;
      }
    }
  ...
  for (mapIter = countMap.begin(); mapIter != countMap.end(); mapIter++) {
    if (mapIter->second > maxNeeded) {
      maxNeeded = mapIter->second;
    }
  }
