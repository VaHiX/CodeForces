/*
 * Problem URL : https://codeforces.com/contest/390/problem/C
...
int main() {
  ...
  scanf("%ld %ld %ld\n", &n, &k, &w);
  std::string candy("");
  getline(std::cin, candy);
  long *cumCount = new long[candy.size() + 1];
  long *cumModCount = new long[candy.size() + 1];

  cumCount[0] = 0;
  cumModCount[0] = 0;
  for (long u = 1; u <= candy.size(); u++) {
    cumCount[u] = cumCount[u - 1] + (candy[u - 1] == '1');
  }
  ...
  for (long u = k; u <= candy.size(); u++) {
    cumModCount[u] = cumModCount[u - k] + (candy[u - 1] == '1');
  }
