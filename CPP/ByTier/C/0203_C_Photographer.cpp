...
int main() {
  ...
  client dummyClient;
  dummyClient.order = 0;
  dummyClient.memory = 0;
  std::vector<client> requestVec(n, dummyClient);

  for (long long p = 0; p < n; p++) {
    long long first, second;
    std::cin >> first >> second;
    requestVec[p].order = p;
    requestVec[p].memory = a * first + b * second;
  }
  ...
  for (long long p = 0; p < served.size(); p++) {
    std::cout << 1 + served[p].order << " ";
  };
  puts("");
