// Problem: CF 2053 I2 - Affectionate Arrays (Hard Version)
// https://codeforces.com/contest/2053/problem/I2

/*
 * Problem: Affectionate Arrays (Hard Version)
 * 
 * Purpose:
 *   This code computes the sum of "value" of all possible valid arrays b that satisfy:
 *   1. Array a is a subsequence of b.
 *   2. The sum of elements in a equals the sum of elements in b.
 *   3. The maximum subarray sum (boredom) of b is minimized.
 *   4. Among such arrays, the length m is minimized.
 *   5. Value of b is the count of times subsequence a appears in b.
 * 
 * Algorithm:
 *   The algorithm uses a deque-based approach combined with dynamic programming ideas to
 *   process the elements of array a and track possible configurations of array b in a way
 *   that optimizes both:
 *   a) The minimal boredom (maximum subarray sum),
 *   b) The minimal length of b.
 *   
 *   The key idea involves maintaining a data structure (deque) that tracks:
 *   - f: current state or offset level
 *   - g: a coefficient or accumulated value relevant to counting subsequences
 *   - len: length of segments in current configuration
 * 
 * Time Complexity: O(n)
 *   Each element in array a is processed once, and operations on the deque are amortized O(1).
 *
 * Space Complexity: O(n)
 *   The deque stores information about segments, and auxiliary arrays hold up to n elements.
 */
...
struct modint {
  ...
  modint operator/(const modint &k) const {
    return Modint::mul(v, Modint::inv(k.v));
  }
  ...
  modint operator/=(const modint &k) {
    return v = Modint::mul(v, Modint::inv(k.v));
  }
};
struct Node {
  int f;
