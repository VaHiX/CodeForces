 ...
 *
 * Approach:
 * 1. Use a sieve to precompute smallest prime factors for efficient
 * factorization
 ...
 *
 * Time Complexity: O(T * (n^2 + u log log u)) where n is array size and u is
 * the maximum value Space Complexity: O(u + n) for sieve and auxiliary arrays
 */
...
int count_multiples[MAX_VALUE]; // count_multiples[d] = count of multiples of d
                                // in the array
