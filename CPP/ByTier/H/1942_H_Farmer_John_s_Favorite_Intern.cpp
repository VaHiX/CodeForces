/*
 * Problem URL : https://codeforces.com/contest/1942/problem/H
...
inline LL Query(Matrix &a)
{
    return max({a.a00, a.a11, a.a20, a.a21, a.a22});
}
...
inline void Insert(int u, int v)
{
    static LL f[3];
    Mul(t[rt[v]].a, f);
    sg[u] += f[0], sh[u] += f[1], sgb[u] += max({f[0], f[1], f[2]}), shb[u] += max(f[1], f[2]);
}
...
{
    Mul(t[t[u].ls].a, t[t[u].rs].a, t[u].a);
}
...
inline void Change(int i)
{
    int u = pos[id[i]];
    Init(t[u].a, i);
    for(u = t[u].fa; u; u = t[u].fa) PushUp(u);
}
...
inline void DFS2(int u, int f)
{
    ...
    if(u == f) Build(rt[u], id[u], id[bot[u]]), t[rt[u]].fa = 0;
}
...
{
    Read(n), Read(m);
    for(int i = 1; i <= n; i ++ ) la[i] = h[i] = g[i] = sg[i] = sh[i] = sgb[i] = shb[i] = son[i] = 0;
    idx = dfst = pid = 0;
    for(int i = 2; i <= n; i ++ )
    {
        int p;
