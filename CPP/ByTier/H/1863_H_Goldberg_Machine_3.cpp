...
template <class T, class... T2> T mmin(T a, T2... b) {
  return mmin(a, mmin(b...));
}
template <class T, class... T2> T mmax(T a, T2... b) {
...
const int lim = 1 << 30;
int xdp[200005];
int merge(int x, int y) { return x == y ? (x << 1 | 1) : (max(x, y) << 1); }
