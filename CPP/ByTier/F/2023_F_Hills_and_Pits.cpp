/*
 * Problem URL : https://codeforces.com/contest/2023/problem/F
...
using namespace std;
const int N=3e5+5;
int T,n,q,a[N],ans[N];long long s[N];
struct node {
	int l,r,id;
...
inline void build(int u,int l,int r) {
	if(l==r) return tr[u]={-1,-1,-1,-1},void();
	int mid=(l+r)>>1;
...
inline void solve() {
	for(int i=1;i<=n;++i) s[i]=s[i-1]+a[i],c[i]={s[i],i};c[n+1]={0,0};
	sort(b+1,b+1+q);sort(c+1,c+2+n);
	build(1,1,n);
	for(int i=1,r,j=1;i<=n+1;i=r+1) {
		for(r=i;r<=n&&c[r+1].x==c[i].x;++r) ;
		for(int j=i;j<=r;++j) if(c[j].id) upd(1,1,n,c[j].id);
		for(;j<=q&&s[b[j].l-1]==c[i].x;++j) ans[b[j].id]=max(ans[b[j].id],query(b[j].l,b[j].r-1));
	}
	return ;
}
inline void work() {
	...
	for(int i=1;i<=q;++i) scanf("%d%d",&b[i].l,&b[i].r),ans[i]=0,b[i].id=i;
	solve();
	for(int i=1;i*2<=n;++i) swap(a[i],a[n+1-i]);
	for(int i=1;i<=q;++i) b[i].l=n+1-b[i].l,b[i].r=n+1-b[i].r,swap(b[i].l,b[i].r);
	solve();
	for(int i=1;i<=q;++i) {
		if(s[b[i].r]<s[b[i].l-1]) ans[b[i].id]=-1;
		else ans[b[i].id]=2*(b[i].r-b[i].l)-ans[b[i].id];
	}
