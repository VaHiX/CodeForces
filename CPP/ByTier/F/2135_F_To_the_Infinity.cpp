...
ull seedA = (ull)chrono::high_resolution_clock::now().time_since_epoch().count();
...
void add(int &p, int pre, int L, int R, int pos, ull v) {
    ...
    if (L == R) return;
    int mid = (L + R) >> 1;
    if (pos <= mid) add(t[p].l, t[pre].l, L, mid, pos, v);
    else add(t[p].r, t[pre].r, mid + 1, R, pos, v);
}

bool cmpSeg(int p, int pre, int L, int R) {
    if (L == R) return t[p].num > t[pre].num;
    int mid = (L + R) >> 1;
    if (t[t[p].r].val == t[t[pre].r].val) return cmpSeg(t[p].l, t[pre].l, L, mid);
    else return cmpSeg(t[p].r, t[pre].r, mid + 1, R);
}
