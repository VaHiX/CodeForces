...
void dfs(int x, int fa, int height, int k, int Tim, int dst, bool f) {
	if (height < 0) Tim = max(Tim, (-height + 1) / 2 * 2 + k * 2 + dst);
	if (Tim <= dist[x]) ans = max(ans, dist[x]);
	else return;
	for (int i = h[x]; ~i; i = ne[i]) {
		int j = e[i];
		if (j == fa) continue;
		if (f && w[j] != w[x]) dfs(j, x, height + w[j], min(k, tim[j]), Tim + 1, dst + 1, 1);
		else dfs(j, x, height + w[j], k, Tim + 1, dst + 1, 0);
	}
}
int main() {
