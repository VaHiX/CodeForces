/*
 * Problem URL : https://codeforces.com/contest/2009/problem/G3
...
void solve() {
  ...
  For (i, n - 1, 0) {
    while (!stk.empty() && f[stk.back()] > f[i]) stk.pop_back();
    if (stk.empty()) {
      rf[i] = f[i] * (n - i);
    } else {
  ...
  while (q--) {
    ...
    l--, r -= k;
    int p = query(l, r);
    cout << (ll)(p - l + 1) * (r - p + 1) * f[p] + rg[l] - rg[p] - rf[p] * (p - l) + lg[r] - lg[p] - lf[p] * (r - p) << '\n';
  }
}
