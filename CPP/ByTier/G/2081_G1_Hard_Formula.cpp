...
// # G1. Hard Formula
// ## Algorithm: Optimized computation of sum of k mod φ(k) for k from 1 to n
// using sieve and inclusion-exclusion principle
...
inline void Add(uint n, const uint &V) {
  // Fenwick tree update: add V to position n and all its ancestors
  while (n <= limit)
    BIT[n] += V, n += n & -n;
}
inline uint Qry(uint n) {
  // Fenwick tree query: get sum from position 1 to n
  uint ans(0);
  while (n)
    ans += BIT[n], n -= n & -n;
  return ans;
...
inline bool check(double P, ull n, uint k) {
  // Check if the current computation is valid based on bounds
  ull x(1);
...
inline bool DFS1(const ull &n, uint k, const ull &phi) {
  // First part of recursive descent for computing sum
  const ull &T = div(N, n);
...
inline void DFS2(const uint &n, uint k, const uint &phi) {
  // Second part of recursive descent - compute values for F and G arrays
...
inline void DFS3(const uint &n, uint k, const uint &phi) {
  // Third part of recursive descent - update BIT structure
...
void sieve(const uint &n) {
  // Sieve of Euler to compute phi values and prime numbers
  phi[1] = 1;
  for (uint i = 2; i <= n; ++i) {
    if (!pos[i]) {
      pri[pos[i] = ++top] = i;
...
void Getphi(const ull &n) {
  // Optimized computation of phi function values for large ranges
  vector<uint> id;
  for (uint i = 1; i <= B; ++i) {
    const ull &w = div(n, i);
    G1[i] = i * (i + 1ull) / 2;
...
inline uint solve(const uint &N) {
  // Simple brute force solution for small N
  uint sum(0);
  phi[1] = 1;
  for (uint i = 2; i <= N; ++i) {
    if (!pos[i])
