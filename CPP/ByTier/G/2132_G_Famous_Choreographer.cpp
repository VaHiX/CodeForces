 ...
 *
 * This program finds the minimum number of additional cells needed to extend a
 * grid such that the original grid and its 180-degree rotation are both present
 ...
 *   - For each test case: O(nm) for preprocessing, O(nm) for checking all
 * shifts
 *
...
int main() {
  ...
  while (testCases--) {
    ...
    // Generate random bases for hashing (with odd numbers for better
    // distribution)
    ...
    vector<ull> powRow1(rows + 1), powRow2(rows + 1), powCol1(cols + 1),
        powCol2(cols + 1);
    ...
    auto getSubrectHash = [&](const vector<ull> &hashTable,
                              ...
                              const vector<ull> &colPows, int rowStart,
                              int rowEnd, int colStart, int colEnd) -> ull {
      int height = rowEnd - rowStart + 1;
      int width = colEnd - colStart + 1;

      ull bottomRight = hashTable[(rowEnd + 1) * (cols + 1) + (colEnd + 1)];
      ull topRight =
          hashTable[rowStart * (cols + 1) + (colEnd + 1)] * rowPows[height];
      ull bottomLeft =
          hashTable[(rowEnd + 1) * (cols + 1) + colStart] * colPows[width];
      ull topLeft = hashTable[rowStart * (cols + 1) + colStart] *
                    rowPows[height] * colPows[width];
    ...
    for (int rowShift = -(rows - 1); rowShift <= (rows - 1); ++rowShift) {
      ...
      for (int colShift = -(cols - 1); colShift <= (cols - 1); ++colShift) {
        ...
        ull origHash1 = getSubrectHash(hash1, powRow1, powCol1, origRowStart,
                                       origRowEnd, origColStart, origColEnd);
        ull rotatedHash1_val =
            getSubrectHash(rotatedHash1, powRow1, powCol1, rotatedRowStart,
                           rotatedRowEnd, rotatedColStart, rotatedColEnd);
        if (origHash1 != rotatedHash1_val)
          continue;

        // Second hash check to minimize collision probability
        ull origHash2 = getSubrectHash(hash2, powRow2, powCol2, origRowStart,
                                       origRowEnd, origColStart, origColEnd);
        ull rotatedHash2_val =
            getSubrectHash(rotatedHash2, powRow2, powCol2, rotatedRowStart,
                           rotatedRowEnd, rotatedColStart, rotatedColEnd);
        if (origHash2 != rotatedHash2_val)
          continue;
