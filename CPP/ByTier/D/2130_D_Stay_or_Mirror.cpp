 ...
 * 
 * Problem: Given a permutation of integers from 1 to n, we need to calculate the
 * minimum number of operations required to sort the permutation. In each operation,
 * we can either keep an element as is or replace it with its mirror position in the sequence.
 * 
 * Approach:
 * 1. For each element in the permutation, count how many elements after it are greater than it.
 * 2. For each element, calculate the minimum between:
 *    - The count of greater elements after it (keeping it as is)
 *    - The count of elements that would be less than it if mirrored (n - v[i] - count)
...
ll solveTestCase(int n, const vector<int>& permutation) {
    ...
    for (int currentIdx = 0; currentIdx < n; ++currentIdx) {
        // Count number of elements after currentIdx that are greater than permutation[currentIdx]
        int greaterAfter = 0;
