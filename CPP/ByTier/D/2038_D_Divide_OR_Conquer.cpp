...
int main() {
  ...
  rep(i, 1, lo[n]) per(j, n, 1) g[j][i] =
      g[j][i - 1] | g[min(j + (1 << (i - 1)), n)][i - 1];
  rep(i, 1, n) {
    v[i].push_back(a[i]);
    int pre = a[i];
    rep(j, 0, (int)v[i - 1].size() - 1) {
      int now = v[i - 1][j] | a[i];
      if (now != pre)
        pre = now, v[i].push_back(now);
    }
    //		for(auto &p:v[i]) cout<<p<<' ';cout<<'\n';
  }
  f[1][0] = 1;
  rep(i, 1, n) rep(j, 0, (int)v[i].size() - 1) if (f[i][j]) {
    int val = v[i][j];
    //		cout<<"ID="<<ID(i+1,val|a[i+1])<<endl;
    if (i < n)
      (f[i + 1][ID(i + 1, val | a[i + 1])] += f[i][j]) %= mod; // suffix
