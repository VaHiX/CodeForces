...
long long luckyEvaluate(std::vector<long long> luckyVec) {
  long long output(0);
  for (int p = luckyVec.size() - 1; p >= 0; p--) {
...
int main() {
  ...
  while (luckyEvaluate(luckyDigits) < left) {
  ...
  while (currentNumber < right) {
    ...
    long long dist;
    if (currentLucky < right) {
      dist = currentLucky - currentNumber;
    } else {
      dist = right - currentNumber;
    }
