...
int main() {
  ...
  for (int before = 0; before < n; before++) {
    ...
    for (int after = before + 1; after < n; after++) {
      ...
      if (timeAfter > timeBefore && timeAfter - timeBefore <= d &&
          sourceBefore == destAfter && destBefore == sourceAfter) {
        if (friends.find(std::pair<std::string, std::string>(
                sourceBefore, destBefore)) == friends.end() &&
            friends.find(std::pair<std::string, std::string>(
                destBefore, sourceBefore)) == friends.end()) {
          friends.insert(
              std::pair<std::string, std::string>(sourceBefore, destBefore));
        }
      }
    }
  }
  ...
  for (pairIter = friends.begin(); pairIter != friends.end(); pairIter++) {
    std::cout << pairIter->first << " " << pairIter->second << std::endl;
  }
