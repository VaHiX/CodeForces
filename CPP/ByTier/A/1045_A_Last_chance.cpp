...
int main(void){
		...
		switch(tpe[i]){
			...
			case 1:{
				...
				while(l<=r){
					...
					if (l&1) addedge(i,n+l,1),l++;

					if (!(r&1)) addedge(i,n+r,1),r--;
	...
	for (int i=1;i<=m;++i){
		...
		if (bel[i]<=n&&tpe[bel[i]]!=1) continue;
		...
		while(!s.empty()&&s.begin()->x.x<i) s.erase(s.begin());

		if (!s.empty()) bel[i]=s.begin()->y,s.erase(s.begin());
