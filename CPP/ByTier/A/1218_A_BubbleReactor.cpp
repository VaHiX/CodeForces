...
namespace Ring
{
	...
	int Dp(int l , int r)
	{
		if((l == 1 && r == tot) || (l > r && r + 1 == l))return 0;
		int &ans = dp[l][r];
		if(ans > 0)return ans;
		auto pr = [&](int l){return l == 1 ? tot : l - 1;};
		auto nx = [&](int r){return r == tot ? 1 : r + 1;};
		int pl = pr(l) , nr = nx(r);
		int st = n - (l <= r ? psz[r] - psz[l - 1] + d[r] - d[l] + 1 - (r - l + 1) 
			: psz[r + tot] - psz[l - 1] + d[r + tot] - d[l] + 1 - (r + tot - l + 1))
			 - Dis(pl) - Dis(r);
		// if(l <= r)cerr << "st = " << n << "-" << (psz[r] - psz[l - 1] + d[r] - d[l] + 1 - (r - l + 1) ) << "-" << Dis(pl) << "-" << Dis(r) << "\n";
		// else cerr << "st = " << n << "-" << psz[r + tot] - psz[l - 1] + d[r + tot] - d[l] + 1 - (r + tot - l + 1) << "-" << Dis(pl) << "-" << Dis(r) << "\n";
		...
		// cerr << st << " +max " << Dp(pl , r) << "+" << F(st - (psz[pl] - psz[pl - 1]) , dl) << " , " << Dp(l , nr) << '+' << F(st - (psz[nr] - psz[nr - 1]) , dr) << "\n";
		// cerr << "dp[" << l << ',' << r << "]:" << ans << "; " << pl << ',' << nr << ' ' << st << "\n";
		return ans;
	}
	int Solve()
	{
		Dfs(1 , 0);
		for(int u : ring)Dfs2(u , 0) , Dfs3(u , 0);
		len = ring.size() , tot = 0;
		if(len == n)
		{
			cout << n * (n + 1) / 2 << "\n";
		...
		for(int j = 0 , i = 0 ; j < len ; j++)
		{
			...
			if(sz[u] == 1)
			{
				...
				{
					// cerr << "j = " << j << ',' << u << " " << l << ',' << r << ":\n";
					// cerr << Dp(l , l) << "+" << tmp << "+" << F(n - Dis(l) - (psz[l] - psz[l - 1]) , Dis(pr(l))) << "\n";
					// cerr << Dp(r , r) << "+" << tmp << "+" << F(n - Dis(l) - (psz[r] - psz[r - 1]) , Dis(r)) << "\n";
					ans = max(ans , Dp(l , l) + tmp + F(n - Dis(l) - (psz[l] - psz[l - 1]) , Dis(pr(l))));
					ans = max(ans , Dp(r , r) + tmp + F(n - Dis(l) - (psz[r] - psz[r - 1]) , Dis(r)));
				}
				else
				{
					// cerr << Dp(l , l) << "+" << ss << "+F(" << n << ',' << Dis(l) + 1 << ")" << F(n , Dis(l) + 1) << "\n";
			...
			else
			{
				...
				ans = max(ans , Dp(i , i) + F(n - sz[u] , dis) 
						+ mxg[u] + ss - sf[u]);
			}
		}
		cout << ans << "\n";
