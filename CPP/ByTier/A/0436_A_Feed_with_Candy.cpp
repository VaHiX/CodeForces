/*
 * Problem URL : https://codeforces.com/contest/436/problem/A
...
long findMaxCandy(std::vector<candy> inputVec, long initialHeight,
                  long initialType) {
  ...
  while (!nothingFound) {
    ...
    for (int p = 0; p < inputVec.size(); p++) {
      if (!inputVec[p].used && inputVec[p].type == targetType &&
          inputVec[p].height <= currentHeight) {
        ++totalCandy;
        nothingFound = 0;
...
int main() {
  ...
  for (int p = 0; p < n; p++) {
    scanf("%ld %ld %ld", &candyVec[p].type, &candyVec[p].height,
          &candyVec[p].mass);
    candyVec[p].used = 0;
  }
