...
bool Wrk(){
  s1=s2=e1=e2=0;
  for(i=0;i<n;i++) S[i]=='A'?++s1:++s2;
  for(i=0;i<m;i++) E[i]=='A'?++e1:++e2;
  if(!(a%2)&&(s1-e1)&1) return false;
  if(!(p%2)&&(s2-e2)&1) return false;
  if(!(a%4)&&!(p%4)){
    int cnt=0;
  ...
  n=strlen(S),s1=s2=0;
  for(i=m;i<n;i++) S[i]=='A'?++s1:++s2;
 
  if(s1||s2){
    if(!s1||!s2) add(m),(!s1)?s1=2:s2=2;
    if(a%4){
      for(i=m;;i++) if(S[i]=='A') break;
      while(s2%p) add(i),++i,s2+=2;
      ...
      for(i=m;;i++) if(S[i]=='P') break;
      while(s1%a) add(i),++i,s1+=2;   
      
      for(i=m;S[i];i++) if(S[i]=='P'){
        j=i;
        while(S[j]=='P') ++j;
        if(S[j]==0) break;
        while((j-i)%p) add(j),++j;
        while(j-i>0) sub(i),j-=p;
      }
      
      if(S[i]=='P'){
        while(j-i>=p) sub(i),j-=p;
        
        while((j-i)%4){
          for(int k=0;k<p;k++) add(i-1+k);
          sub(i-1),j+=p;
        }
                
        for(int t=0;t<(j-i)/4;t++){
          for(int k=0;k<a-1;k++) add(i+2*t+k);
          sub(i+2*t-1),add(i+2*t+a-1),sub(i+2*t);
        }
        j=i-1+(j-i)/2,--i;
        while((j-i)%p) add(j),++j;
        while(j-i>=p) sub(j+1),sub(i),j-=p;
      }
      while(s1) sub(m),s1-=a;
    }
  }
