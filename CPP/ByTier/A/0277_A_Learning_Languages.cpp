/*
 * Problem URL : https://codeforces.com/contest/277/problem/A
      ...
      for (int q = 0; q < languages.size(); q++) {
        if (connected[p].find(languages[q]) != connected[p].end()) {
          for (int r = 0; r < languages.size(); r++) {
            connected[p].insert(languages[r]);
          }
          belongs = 1;
          break;
        }
  ...
  if (connected.size() > 0) {
    ...
    for (int p = 0; p < connected.size(); p++) {
      for (int q = p + 1; q < connected.size(); q++) {
        ...
        for (myIter = connected[q].begin(); myIter != connected[q].end();
             myIter++) {
          if (connected[p].find(*myIter) != connected[p].end()) {
            unmerged[q] = 0;
            break;
          }
        }
        if (!unmerged[q]) {
          for (myIter = connected[p].begin(); myIter != connected[p].end();
               myIter++) {
            connected[p].insert(*myIter);
          }
        }
      }
    }

    for (int p = 0; p < unmerged.size(); p++) {
      if (unmerged[p]) {
        ++total;
      }
    }
