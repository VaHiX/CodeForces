/*
 * Problem URL : https://codeforces.com/contest/182/problem/A
...
long long PointToLine(const TPoint &P, const TLine &L) {
  if (L.A.x <= P.x && P.x <= L.B.x)
    return abs(P.y - L.A.y) * abs(P.y - L.A.y);
  if (L.A.y <= P.y && P.y <= L.B.y)
    return abs(P.x - L.A.x) * abs(P.x - L.A.x);
