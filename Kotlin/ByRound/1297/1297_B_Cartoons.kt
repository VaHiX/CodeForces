...
object programkt {
  ...
  internal class TaskB {
    fun solve(testNumber: Int, `in`: InputReader, out: OutputWriter) {
      ...
      // Check all start days of cartoons
      for (i in a) {
        var q = 0
        // Count how many cartoons are airing on day i
        for (j in 0 until n) {
          if (i >= a[j] && i <= b[j]) {
            q++
          }
        }
        // If exactly one cartoon is airing, return this day
        if (q == 1) {
          out.printLine(i)
          return
        }
      }
      // Check all end days + 1 of cartoons
      for (k in b) {
        val i = k + 1
        var q = 0
        // Count how many cartoons are airing on day i
      ...
      // No day found when exactly one cartoon is airing
      out.printLine(-1)
    }
  }
