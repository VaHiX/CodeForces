// Problem: CF 1297 G - M-numbers
// https://codeforces.com/contest/1297/problem/G
...
object programkt {
  ...
  internal class TaskG {
    ...
    // Dynamic programming function to count m-numbers of a given length
    // with given prime factor powers
    fun qty(len: Int, a2: Int, a3: Int, a5: Int, a7: Int): Long {
      // Extend the memoization array if needed
      while (res.size <= len) {
      ...
      // Base case: 0 length means 1 way (empty string)
      if (len == 0) {
        slice[a2][a3][a5][a7] = (if (a2 == 0 && a3 == 0 && a5 == 0 && a7 == 0) 1 else 0).toLong()
        return slice[a2][a3][a5][a7]
      }
      slice[a2][a3][a5][a7] = 0
      // Try each possible digit (1 to 9) and compute the count recursively
      for (v in vals) {
    ...
    fun solve(testNumber: Int, `in`: InputReader, out: OutputWriter) {
      var m = `in`.readInt()
      var k = `in`.readLong() - 1 // Convert to 0-indexed
      p2 = 0
      p3 = 0
      p5 = 0
      p7 = 0
      // Factorize m into powers of 2, 3, 5, 7
      while (m % 2 == 0) {
        m /= 2
        p2++
      }
      ...
      // If m has other prime factors, no solution exists
      if (m != 1) {
        out.printLine(-1)
        return
      }
      var len = 1
      // Find the number of digits needed to be able to form k-th m-number
      while (true) {
        ...
        // Construct the k-th m-number digit by digit
        ...
        for (i in 0 until len) {
          for (j in 0..8) {
            ...
            if (p2 >= v[0] && p3 >= v[1] && p5 >= v[2] && p7 >= v[3]) {
              ...
              // Skip combinations that are still smaller than k
              if (q <= k) {
                k -= q
                continue
              }
              // Place this digit and update the remaining prime powers
