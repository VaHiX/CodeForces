// Contest 1431, Problem H: Rogue-like Game
// URL: https://codeforces.com/contest/1431/problem/H
...
fun main() {
  ...
  val AOpen =
      (0 until N).map { i -> A[i] to (0 until B.count { it <= A[i] }).map { j -> C[i][j] }.max()!! }
  val BOpen =
      (0 until M).map { i -> B[i] to (0 until A.count { it <= B[i] }).map { j -> C[j][i] }.max()!! }
