# Contest 1853, Problem E: Ina of the Mountain
# URL: https://codeforces.com/contest/1853/problem/E
...
def A2(n, m, x=0):
    return [[x] * m for i in range(n)]


def A3(a, b, c, x=0):
    return [[[x] * c for j in range(b)] for _ in range(a)]
    ...
    def __init__(self, op, e, arr):  # n!=0
        ...
        for j in range(1, self.mx):
            for i in range(self.n - (1 << j) + 1):
                self.st[j * self.n + i] = op(
                    self.st[i + (j - 1) * self.n],
                    self.st[i + (1 << j - 1) + (j - 1) * self.n],
                )
...
class dheap:
    ...
    def remove(self, x):
        self.c[x] -= 1
        while self.heap and self.c[self.heap[0]] == 0:
            heappop(self.heap)
