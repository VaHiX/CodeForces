# Contest 2025, Problem F: Choose Your Queries
# URL: https://codeforces.com/contest/2025/problem/F
...
for u in order:
    cur = -1
    for eid in path[u]:
        if dirs[eid] == -1 and parent[u] != eid:
            if cur == -1:
                cur = eid
            else:
...
for i in range(q):
    outs.append("yx"[dirs[i]] + "+-"[cur[us[i]] if dirs[i] else cur[vs[i]]])
    if dirs[i]:
        cur[us[i]] ^= 1
    else:
        cur[vs[i]] ^= 1
