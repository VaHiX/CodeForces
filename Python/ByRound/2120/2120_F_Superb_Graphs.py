...
def solve_case():
    ...
    for _ in range(k):
        ...
        hashes1 = [
            sum(adj[i][j] * powers1[j] for j in range(n)) % MOD1 for i in range(n)
        ]
        hashes2 = [
            sum(adj[i][j] * powers2[j] for j in range(n)) % MOD2 for i in range(n)
        ]
        ...
        for u in range(n):
            for v in range(u + 1, n):
                ...
                h_u1 = (
                    hashes1[u] - (adj[u][u] * powers1[u]) - (adj[u][v] * powers1[v])
                ) % MOD1
                h_v1 = (
                    hashes1[v] - (adj[v][u] * powers1[u]) - (adj[v][v] * powers1[v])
                ) % MOD1

                h_u2 = (
                    hashes2[u] - (adj[u][u] * powers2[u]) - (adj[u][v] * powers2[v])
                ) % MOD2
                h_v2 = (
                    hashes2[v] - (adj[v][u] * powers2[u]) - (adj[v][v] * powers2[v])
                ) % MOD2
