# Contest 1866, Problem J: Jackets and Packets
# URL: https://codeforces.com/contest/1866/problem/J
...
for step in range(n):
    for l in range(1, n - step + 1):
        ...
        for sep in range(l, r):
            dp[0][f(l, r)] = fmin(
                dp[0][f(l, r)], dp[0][f(l, sep)] + dp[0][f(sep + 1, r)]
            )

        if step and nums[l] == nums[r]:
            dp[1][f(l, r)] = dp[0][f(l + 1, r - 1)] + 2 * y
            dp[2][f(l, r)] = dp[0][f(l + 1, r - 1)] + 4 * y

            for sep in range(l + 1, r):
                if nums[l] == nums[sep]:
                    dp[1][f(l, r)] = fmin(
                        dp[1][f(l, r)], dp[1][f(l, sep)] + dp[0][f(sep + 1, r - 1)] + y
                    )
                    dp[2][f(l, r)] = fmin(
                        dp[2][f(l, r)],
                        dp[2][f(l, sep)] + dp[0][f(sep + 1, r - 1)] + 2 * y,
                    )

            dp[0][f(l, r)] = fmin(dp[0][f(l, r)], dp[1][f(l, r)] + x)

            for sep in range(r, l - 1, -1):
                if nums[sep] != nums[r]:
                    break
