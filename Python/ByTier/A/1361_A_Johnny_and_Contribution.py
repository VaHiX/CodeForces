...
class RangeQueue:
    ...
    def mn(self, st):
        return st[-1][1] if st else 0  # gcd or max= or sum0,mi=inf

    def get_fn(self):
        return self.fn(self.mn(self.inbox), self.mn(self.outbox))
    ...
    def push(self, v):
        if not self.inbox:
            self.inbox.append((v, v))
        else:
            self.inbox.append((v, self.fn(v, self.mn(self.inbox))))
    ...
    def shift(self):
        while self.inbox:
            self.outbox.append(
                (self.inbox[-1][0], self.fn(self.inbox[-1][0], self.mn(self.outbox)))
            )
            self.inbox.pop()
...
class SortedList:
    def __init__(A, iterable=[], _load=200):
        B = _load
        C = sorted(iterable)
        A._len = E = len(C)
        A._load = B
        A._lists = D = [C[A : A + B] for A in range(0, E, B)]
        A._list_lens = [len(A) for A in D]
        A._mins = [A[0] for A in D]
        A._fen_tree = []
        A._rebuild = _A

    def _fen_build(B):
        B._fen_tree[:] = B._list_lens
        C = B._fen_tree
    ...
    def remove(A, value):
        ...
        A.discard(B)
        if C == A._len:
            raise ValueError("{0!r} not in list".format(B))

    def pop(A, index=-1):
        B = index
        C, D = A._fen_findkth(A._len + B if B < 0 else B)
        E = A._lists[C][D]
        A._delete(C, D)
        return E

    def bisect_left(A, value):
        B, C = A._loc_left(value)
        return A._fen_query(B) + C

    def bisect_right(A, value):
        B, C = A._loc_right(value)
        return A._fen_query(B) + C

    def count(A, value):
        B = value
        return A.bisect_right(B) - A.bisect_left(B)
    ...
    def __contains__(B, value):
        ...
        if A:
            D, E = B._loc_left(C)
            return E < len(A[D]) and A[D][E] == C
        return False

    def __iter__(A):
        return (B for A in A._lists for B in A)

    def __reversed__(A):
        return (B for A in reversed(A._lists) for B in reversed(A))

    def __repr__(A):
        return "SortedList({0})".format(list(A))
