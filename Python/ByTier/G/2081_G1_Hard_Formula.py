# Contest 2081, Problem G1: Hard Formula
...
"""
TL 2sec
k mod phi(k) の総和 mod 2^32 を求めよ N <= 10^10, 10^12

k = prod p^ei 
phi(k) = prod (p-1) p^(ei-1)

素因数集合Sごとにまとめてみる？
→固定すると phi(k) = kt iff k = phi(k)1/t の t が固定  -> [1/t] が固定される
→固定すると k,phi(k) の総和を求める問題になる

1/t = prod p/(p-1)

篩みたいにしたいけど 1/t持たないといけないのきつすぎ…
→black algorithmできますね
"""
...
def solve(N):
    mod = 2**32 - 1
    """
    calc prime_sum(N//i),prime_sum(i) for i=1,2,...,√N 
    """
