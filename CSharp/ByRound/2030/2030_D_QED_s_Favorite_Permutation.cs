...
public class Solver
{
    ...
    void Solve()
    {
        ...
        var lmax = new int[n];
        var rmin = new int[n];
        a.AsSpan().CopyTo(lmax);
        a.AsSpan().CopyTo(rmin);
        for (int i = 1; i < lmax.Length; i++) Chmax(ref lmax[i], lmax[i - 1]);
        for (int i = rmin.Length - 1; i > 0; i--) Chmin(ref rmin[i - 1], rmin[i]);

        int c = 0;
        void F(int x, int v)
        {
            if (0 <= x && x + 1 < n && s[x] == 'L' && s[x + 1] == 'R' && lmax[x] > rmin[x + 1]) c += v;
        }
