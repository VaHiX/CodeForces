...
public class Solver
{
    ...
    void Solve()
    {
        ...
        var c = new List<int>();
        for (int i = 0; i < n; i++) if (a[i] <= m) c.Add(a[i]);

        bool ans = true;
        for (int x = 0, y = c.Count - 1; x < y;)
        {
            while (x < c.Count && c[x] == m) x++;
            while (y >= 0 && c[y] == m) y--;
            if (x >= y) break;
            ans &= c[x] == c[y];
            x++; y--;
        ...
        {
            int cnt = 0;
            for (int x = 0, y = c.Count - 1; ;)
            {
                int cx = 0, cy = 0;
                while (x < c.Count && c[x] == m) { x++; cx++; }
                while (y >= 0 && c[y] == m) { y--; cy++; }
                if (x <= y) cnt += Math.Min(cx, cy) * 2;
                else cnt += cx;
                if (x >= y) break;
                x++; y--;
            }
