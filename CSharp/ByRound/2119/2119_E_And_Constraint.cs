...
public class Solver
{
    ...
    {
        int n = ri;
        ReadArray(out long[] a, n - 1);
        ReadArray(out long[] b, n);
        ...
        bool ok = true;
        for (int i = 0; i < n - 1; i++) ok &= a[i] >= (x[i] & x[i + 1]);
        long ans = 0;
        if (!ok)
        {
            ans = -1;
        ...
        else
        {
            ...
            var z = new long[n];
            for (int i = 0; i < n; i++) z[i] = Math.Max(0, b[i] - x[i]);
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                for (int k = 32; k >= 0; k--)
                {
                    if (((y[i] >> k) & 1) == 1) continue;
                    if ((v | (((1L << k) - 1) & ~y[i])) < z[i]) v |= 1L << k;
                }

                int l = 0;
                var ndp = new long[40];
                var nval = new long[40];
                while (v < (1L << 32))
                {
                    long w = INF;
                    for (int j = 0; j < m; j++)
                    {
                        if ((val[j] & v) != 0) continue;
                        if (dp[j] + v < w) w = dp[j] + v;
                    }
                    ndp[l] = w;
                    nval[l] = v;
                    l++;
                    ...
                    v += v == 0 ? 1 : 1L << Ctz(v);
                    for (int j = 0; j < 40; j++)
                        if (((y[i] >> j) & 1) == 1 && ((v >> j) & 1) == 1) v += 1L << j;
                }
                dp = ndp;
                val = nval;
                m = l;
            }
            ans = dp[..m].Min() + x.Sum() - b.Sum();
        }
        Write(ans);
    }
