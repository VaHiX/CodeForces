...
public class Solver
{
    ...
    void Solve()
    {
        ...
        int l = 0;
        while ((1 << l) <= n) l++;
        if (n % 2 == 1) { ans.Add(n); used[n] = true; }
        for (int i = l - 1; i >= 2; i--)
        {
            int v = 1 << i;
            ans.Add(v);
        ...
        ans.Add(2); used[2] = true;
        ans.Add(5); used[5] = true;
        ans.Add(1); used[1] = true;

        for (int i = 1; i <= n; i++) if (!used[i]) ans.Add(i);
        ans.Reverse();

        int k = 0;
        for (int i = 0; i < n; i++)
            k = i % 2 == 0 ? (k & ans[i]) : (k | ans[i]);
        Write(k);
        WriteJoin(" ", ans);
    }
