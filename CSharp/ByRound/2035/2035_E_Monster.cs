    ...
    void Solve()
    {
        ...
        if (k * k <= z)
        {
            ...
            for (long d = 1; z > 0; d++)
            {
                v += x;
                if (d % k == 0) { z -= d; v += y; }
                Chmin(ref ans, v + Ceil(Math.Max(0, z), d) * y);
            }
        }
        else
        {
            ...
            {
                long w = z - k * l * (l + 1) / 2;
                long left = l * k, right = (l + 1) * k;
                long d = 1;
                for (; Ceil(w, d) > Ceil(w, d + 1); d++)
                    if (left <= d && d < right) Chmin(ref ans, l * y + d * x + Ceil(w, d) * y);
                for (long v = Ceil(w, d); v > 0; v--)
                {
                    long l1 = Ceil(w, v), r1 = v == 1 ? INF : Ceil(w, v - 1);
                    Chmax(ref l1, left);
                    Chmin(ref r1, right);
                    if (l1 < r1)
                    {
                        d = l1;
                        Chmin(ref ans, l * y + d * x + Ceil(w, d) * y);
                    }
                }
                if (w <= 0)
                {
                    Chmin(ref ans, l * y + l * k * x);
                    break;
                }
            }
        }
