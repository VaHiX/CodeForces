...
public class Solver
{
    ...
    void Solve()
    {
        ...
        if (k % 2 == 0)
        {
            ...
            for (int i = 0; i < k / 2; i++)
            {
                for (int j = 0; j < n; j++) a[j] = p[j] + 1;
                WriteJoin(" ", a);
                for (int j = 0; j < n; j++) a[j] = n - p[j];
                WriteJoin(" ", a);
                if (i == k / 2 - 1) break;
                p.Next();
            }
        ...
        YN(true);
        {
            ...
            for (int i = m; i < n; i++) v[i] = i - m + 1;
            var w = new int[n];
            for (int i = 0; i < n; i++) w[i] = 3 * (n + 1) / 2 - u[i] - v[i];
            WriteJoin(" ", u);
            WriteJoin(" ", v);
            WriteJoin(" ", w);
            ...
            {
                for (int j = 0; j < n; j++) a[j] = p[j] + 1;
                for (int j = 0; j < n; j++) b[j] = n - p[j];
                bool ok = true;
                {
                    bool f = true;
                    for (int j = 0; j < n; j++) f &= a[j] == u[j];
                    ok &= !f;
                }
                {
                    bool f = true;
                    for (int j = 0; j < n; j++) f &= a[j] == v[j];
                    ok &= !f;
                }
                {
                    bool f = true;
                    for (int j = 0; j < n; j++) f &= a[j] == w[j];
                    ok &= !f;
                }
                {
                    bool f = true;
                    for (int j = 0; j < n; j++) f &= b[j] == u[j];
                    ok &= !f;
                }
                {
                    bool f = true;
                    for (int j = 0; j < n; j++) f &= b[j] == v[j];
                    ok &= !f;
                }
                {
                    bool f = true;
                    for (int j = 0; j < n; j++) f &= b[j] == w[j];
                    ok &= !f;
                }
                if (ok)
                {
