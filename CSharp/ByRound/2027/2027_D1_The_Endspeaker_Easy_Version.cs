...
public class Solver
{
    ...
    void Solve()
    {
        ...
        var dp = new SegTree<long, LongMinMonoid>[m];
        for (int i = 0; i < m; i++)
        {
            dp[i] = new SegTree<long, LongMinMonoid>(n + 1);
            dp[i].Set(n, 0);
        }
        var sum = new long[m];
        ...
        for (int i = n - 1; i >= 0; i--)
        {
            ...
            for (int j = m - 1; j >= 0; j--)
            {
                ...
                Chmin(ref min, Math.Min(INF, dp[j].Prod(i, right[j] + 1)) - j);
                long v = min + m - 1;
                dp[j].Set(i, v);
            }
        }
        ...
        long ans = INF;
        for (int j = 0; j < m; j++) Chmin(ref ans, dp[j].Get(0));
        if (ans >= INF / 2) ans = -1;
        Write(ans);
    }
