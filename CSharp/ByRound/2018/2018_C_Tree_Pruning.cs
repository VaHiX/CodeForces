...
public class Solver
{
    ...
    void Solve()
    {
        ...
        par[0] = -1;
        var depth = new int[n];
        var depth_max = new int[n];

        var st = new SimpleIntStack(n);
        var dfs_ord = new int[n];
        {
            int ord_idx = 0;
            st.Push(0);
            while (st.Count > 0)
            ...
            {
                int x = dfs_ord[i];
                Chmax(ref depth_max[par[x]], depth_max[x]);
            }
        }
...
public class SimpleIntStack{int[]data;int index;public SimpleIntStack(int size_max){data=new int[size_max];index=0;}public int Count{get=>index;}public void Push(int value)=>data[index++]=value;public int Peek()=>data[index-1];public int Pop()=>data[ --index];}public class SimpleIntPairStack{(int,int)[]data;int index;public SimpleIntPairStack(int size_max){data=new(int,int)[size_max];index=0;}public int Count{get=>index;}public void Push((int,int)value)=>data[index++]=value;public void Push(int item1,int item2)=>data[index++]=(item1,item2);public(int,int)Peek()=>data[index-1];public(int,int)Pop()=>data[ --index];}
