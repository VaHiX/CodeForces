...
public class Solver
{
    ...
    void Solve()
    {
        ...
        var par = new int[n];
        par[0] = -1;
        for (int i = 1; i < n; i++) par[i] = ri - 1;
        var p = new int[n];
        for (int i = 0; i < n; i++) p[i] = ri;

        var dfs_ord_par = new int[n];
        int DFS(int x, int ord, int par_ord)
        {
            dfs_ord_par[ord] = par_ord;
            par_ord = ord++;
            if (2 * x + 1 <= n)
            {
                ord = DFS(2 * x, ord, par_ord);
                ord = DFS(2 * x + 1, ord, par_ord);
            }
            return ord;
        }
        DFS(1, 0, -1);
        var dfs_ord_child = new int[n, 2];
        for (int i = 0; i < n; i++) for (int j = 0; j < 2; j++) dfs_ord_child[i, j] = -1;
        for (int i = 0; i < n; i++)
            if (dfs_ord_par[i] != -1)
            {
                int x = dfs_ord_par[i];
                dfs_ord_child[x, dfs_ord_child[x, 0] == -1 ? 0 : 1] = i;
            }

        int WrongPoint(int x)
        {
            if (x == -1 || dfs_ord_par[x] == -1) return 0;
            int y = dfs_ord_par[x];
            return p[y] == p[x] / 2 ? 0 : 1;
        }
        int Wrong(int x) => WrongPoint(x) + WrongPoint(dfs_ord_child[x, 0]) + WrongPoint(dfs_ord_child[x, 1]);

        int wrong = 0;
        for (int x = 0; x < n; x++) wrong += WrongPoint(x);
        ...
        while (q-- > 0)
        {
            ...
            wrong -= Wrong(x);
            wrong -= Wrong(y);
            if (dfs_ord_par[y] == x) wrong += WrongPoint(y);
