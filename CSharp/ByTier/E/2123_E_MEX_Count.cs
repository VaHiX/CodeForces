...
public class Program
{
    ...
    public static void MainProblem()
    {
        ...
        int[] prefixCounts = counts.PrefixSum();

        int mex = 0;

        for (; mex <= n && counts[mex] > 0; mex++)
        {
        }
        ...
        {
            long add = counts[i];
            long rem = n - prefixCounts[i + 1] + (prefixCounts[i + 1] - i);
            answ[add]++;
            answ[rem]--;
        }
