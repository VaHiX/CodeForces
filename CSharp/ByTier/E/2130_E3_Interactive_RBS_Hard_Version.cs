...
namespace CodeForces.Problems.Round2130.E3
{
    /// <summary>
    /// Problem: CF2130E3 - Interactive RBS (Hard Version)
    /// 
    ...
    /// This is the hard version with the most stringent constraints.
    /// 
    /// Approach:
    ...
    /// - Optimize for the hard version's constraints
    /// 
    ...
    /// 
    /// Constraints (Hard Version):
    /// - 1 ≤ t ≤ 100 (number of test cases)
    /// - 1 ≤ n ≤ 8000 (length of the bracket sequence, larger than medium version)
    /// - Maximum 540 queries per test case
    /// - More complex bracket patterns than E1 and E2
    /// </summary>
    public class InteractiveRBSHardSolver
    {
        ...
        /// <summary>
        /// Initializes a new instance of the InteractiveRBSHardSolver class
        /// </summary>
        public InteractiveRBSHardSolver(TextReader input, TextWriter output)
        {
        ...
        public static void Main()
        {
            ...
            using var solver = new InteractiveRBSHardSolver(
                Console.In,
        ...
        private void Solve()
        {
            ...
            if (n <= 0)
            {
                throw new ArgumentException($"Invalid input: n must be positive, got {n}", nameof(n));
            }
        ...
        private void ProcessBracketSequence(int n)
        {
            ...
            var currentIndices = Enumerable.Range(2, Math.Min(32, n - 1)).ToList();
            ...
            while (!result.IsFound)
            {
                ...
                currentIndices = Enumerable
                    .Range(result.LastProcessedIndex + 1, Math.Min(32, n - result.LastProcessedIndex - 1) + 1)
                    .Where(x => x <= n)
                    .ToList();
        ...
        private void ProcessRemainingBrackets(int n, int openBracketPos)
        {
            ...
            foreach (int index in undeterminedIndices)
            {
                ...
                if (currentQuery.Count + 2 * _chunkSizes[chunkIndex] + 1 < 1000)
                {
                    ...
                    chunkIndex = Math.Min(chunkIndex + 1, _chunkSizes.Count - 1);
                }
                else
                {
        ...
        private void ProcessQueryResult(long queryResult, List<int> indices)
        {
            ...
            {
                if (i >= _chunkWeights.Count) continue; // Safety check
