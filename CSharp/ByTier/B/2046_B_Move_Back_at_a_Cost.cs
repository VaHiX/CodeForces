...
public class Solver
{
    ...
    void Solve()
    {
        ...
        min = IINF;
        for (int i = 0; i < n; i++) if (f[i]) Chmin(ref min, a[i] + 1);
        for (int i = 0; i < n; i++) f[i] |= a[i] > min;

        var ans = new List<int>();
        for (int i = 0; i < n; i++) if (!f[i]) ans.Add(a[i]);
        var b = new List<int>();
        for (int i = 0; i < n; i++) if (f[i]) b.Add(a[i]);
        b.Sort();
        for (int i = 0; i < b.Count; i++) ans.Add(b[i] + 1);
        WriteJoin(" ", ans);
    }
