...
public class Solver
{
    ...
    void Solve()
    {
        ...
        var st = new SparseTable<int>(diff, Gcd);
        int Prod(int l, int r) => l < r ? st.Prod(l, r) : 0;

        long ans = 0;
        int mr = 0;
        var r_geq = new int[32];
        var r_gt = new int[32];
        for (int l = 0; l < n; l++)
        {
            Chmax(ref mr, l);
            while (mr < n && Prod(l, mr) == 0) mr++;
            ans += mr - l;
            for (int d = 31; d >= 0; d--)
            {
                long t = 1L << d;
                Chmax(ref r_geq[d], mr);
                while (r_geq[d] < n)
                {
                    var v = Prod(l, r_geq[d]);
                    if (v == 0 || v >= t) r_geq[d]++; else break;
                }
                Chmax(ref r_gt[d], mr);
                while (r_gt[d] < n)
                {
                    var v = Prod(l, r_gt[d]);
                    if (v == 0 || v > t) r_gt[d]++; else break;
                }
                ans += r_geq[d] - r_gt[d];
            }
        }
    ...
    struct Op : IMonoid<int>
    {
        public int Identity => 0;
        public int Operate(int x, int y) => Math.Abs(Gcd(x, y));
    }
