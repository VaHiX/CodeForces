...
public class Program
{
    ...
    {
        (int n, int k) = Reader.InputTuple2<int>();
        long[] a = Reader.InputArray<long>();
        long[] b = Reader.InputArray<long>();
        ...
        for (int i = 0; i < n; i++)
        {
            c[i] = (Math.Min(a[i], b[i]), Math.Max(a[i], b[i]));
            answ += Math.Max(a[i], b[i]) - Math.Min(a[i], b[i]);
        }

        var xd = c.OrderBy(a => a.Item1).ThenBy(a => a.Item2).ToArray();
        ...
        //Console.WriteArray(newMin);
        //Console.WriteArray(newMax);

        long delta = long.MaxValue;

        for (int i = 0; i < n; i++)
        {
            long target = newMax[i];

            int index = newMin.LowerBoundIndex(target);
            ...
            long curAnsw = long.MaxValue;
            if (index < n)
            {
                long old = Math.Abs(newMax[i] - newMin[i]) + Math.Abs(newMax[index] - newMin[index]);

                long cur = Math.Abs(newMax[index] - newMin[i]) + Math.Abs(newMax[i] - newMin[index]);

                curAnsw = Math.Min(curAnsw, cur - old);
            }

            if (min1 >= 0)
            {
                long old = Math.Abs(newMax[i] - newMin[i]) + Math.Abs(newMax[min1] - newMin[min1]);

                long cur = Math.Abs(newMax[min1] - newMin[i]) + Math.Abs(newMax[i] - newMin[min1]);

                curAnsw = Math.Min(curAnsw, cur - old);
            }

            delta = Math.Min(delta, curAnsw);
        }

        Console.WriteLine(delta + answ);
    }
}