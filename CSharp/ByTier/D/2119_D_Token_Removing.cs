...
// Purpose: Solve the "Token Removing" problem using dynamic programming.
// Algorithms: Dynamic Programming with memoization, modular arithmetic
...
public class Solver {
  ...
  void Solve() {
    ...
    for (int k = n; k > 0; k--) {
      for (int i = n - k; i >= 0; i--) {
        // Calculate the value for dp[i+1] based on the recurrence relation
        // dp[i+1] = (dp[i+1] + dp[i] * (n + 1 - k - i) * k) % m
        long v = dp[i + 1] + dp[i] * (n + 1 - k - i) % m * k % m;
        if (v >= m)
          v -= m;
        dp[i + 1] = v;
      }
    }
    // Sum all values in dp array and take modulo m
    long ans = dp.Sum() % m;
    Write(ans);
  }
