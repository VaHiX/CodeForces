...
public class Program
{
    ...
    public static int[] MaxFactor = new int[200_010];
    ...
    {
        for (int i = 2; i < MaxFactor.Length; i++)
        {
            if (MaxFactor[i] != 0)
                continue;
            ...
            for (int j = 2 * i; j < MaxFactor.Length; j += i)
            {
                MaxFactor[j] = i;
            }
        }
    ...
    public static void MainProblem()
    {
        ...
        {
            int ind = i + 1;
            int maxF = MaxFactor[ind] - 1;

            (res[maxF], res[i]) = (res[i], res[maxF]);
        }

        Console.WriteArray(res);
    }
}